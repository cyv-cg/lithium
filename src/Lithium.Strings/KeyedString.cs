...
public sealed class KeyedString {
	...
	public KeyedString(string address) {
		(string @namespace, string key) = TranslationService.ParseAddress(address);
		Namespace = @namespace;
		Key = key;
	}
	...
	public string Translate(params StringArgument[] values) {
		if (TranslationService.Default != null) {
			return Translate(TranslationService.Default, values);
		}
		return Address;
	}
	/// <summary>
	...
	/// If the string has not been loaded into the context, returns the string address instead.
	/// </summary>
	/// <param name="service">The service used to translate the string.</param>
	/// <param name="values">String parameters.</param>
	/// <returns>Translated string with parameters replaced.</returns>
	/// <exception cref="StringTranslationException">Thrown when there is an error during translation or interpolation.</exception>
	/// <exception cref="ArgumentException">Thrown when an argument key is null or empty.</exception>
	/// <exception cref="ArgumentNullException">Thrown when the address is an empty string.</exception>
	public string Translate(ITranslationService service, params StringArgument[] values) {
		if (IsLoaded(service)) {
			return service.Translate(Address, values);
		}
		return Address;
	}
	...
	/// <exception cref="ArgumentException">Thrown when an argument key is null or empty.</exception>
	/// <exception cref="ArgumentNullException">Thrown when the address is an empty string.</exception>
	public string ToString(ITranslationService service, params StringArgument[] values) {
		return Translate(service, values);
	}
