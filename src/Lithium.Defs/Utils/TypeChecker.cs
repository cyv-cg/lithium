...
internal static class TypeChecker {
	...
	/// The constructor or method must have the <see cref="XmlFactory"/> attribute and take a single
	/// <see cref="XmlNode"/> parameter.
	...
	internal static bool IsSpecialConstructor(this Type type, XmlNode node, [NotNullWhen(true)] out MethodBase? factory) {
		...
		factory = type.GetConstructors().FirstOrDefault(c => c.GetCustomAttribute<XmlFactory>() != null);
		// Then look for a factory.
		factory ??= type.GetMethods().FirstOrDefault(m => m.GetCustomAttribute<XmlFactory>() != null);
		if (factory != null) {
